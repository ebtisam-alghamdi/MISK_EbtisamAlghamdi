...
*The highest food category with portin?
*
...
# Remove t in (fat, portin, sat.fat and fiber) replace it with "0"
...
```{r}
 nfc.new <-
nfc %>% 
  mutate(highprotan=max(Protein, na.rm = TRUE))
```
...
nfc %>% 
group_by(Category) %>%
summarise(n = n(),max_cal = max(Fiber, na.rm = TRUE))
...
# Show the Calories in each Category 

ggplot(nfc, aes( Calories,Category ,color= Category)) +
